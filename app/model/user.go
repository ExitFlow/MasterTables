...
type User struct {
	...
	IdEjecutivo string        `db:"idejecutivo" bson:"idejecutivo"`
	StatusID    uint8         `db:"status_id" bson:"status_id"`
	CreatedAt   time.Time     `db:"created_at" bson:"created_at"`
	UpdatedAt   time.Time     `db:"updated_at" bson:"updated_at"`
	Deleted     uint8         `db:"deleted" bson:"deleted"`
	IdSystem    uint32        `db:"idsystem" bson:"idsystem"`
}
...
func UserByEmail(email string) (User, error) {
	...
	switch database.ReadConfig().Type {
	case database.TypeMySQL:
		err = database.SQL.Get(&result, "SELECT id, password, status_id, first_name, idejecutivo FROM sec_user WHERE email = ? and idsystem = 5 LIMIT 1;", email)
	case database.TypeMongoDB:
...
func UserCreate(firstName, lastName, email, password string) error {
	...
	switch database.ReadConfig().Type {
	case database.TypeMySQL:
		_, err = database.SQL.Exec("INSERT INTO sec_user (first_name, last_name, email, password, cod_user, idsystem) VALUES (?,?,?,?,?,2)", firstName,
			lastName, email, password, firstName[1:1]+lastName)
	case database.TypeMongoDB:
		if database.CheckConnection() {
			session := database.Mongo.Copy()
			defer session.Close()
